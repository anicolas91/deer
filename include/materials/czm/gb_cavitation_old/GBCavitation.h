 ...
 */
class GBCavitation : public CZMMaterialBaseStateful {
public:
...
protected:
  ...
  virtual void
  computeTractionDerivativesAndNewMaterialPropertiesLocal() override;
  ...
  const InterfaceQpMaterialPropertyRankTwoScalarUO &_avg_mises_stress_uo;
  ...
  const InterfaceQpMaterialPropertyRankTwoScalarUO &_avg_hyd_stress_uo;
  ...
  const InterfaceQpMaterialPropertyRankTwoScalarUO &_avg_eq_strain_rate_uo;
  ...
  void update_Dtn_dUN(const nlFunBase::io_maps_type &params,
    ...
    // for (unsigned int i = 0; i < 3; i++)
    _traction_derivatives[_qp](0, 0) = dx_dy(2);
    _traction_derivatives[_qp](0, 1) = 0;
    _traction_derivatives[_qp](0, 2) = 0;
  }
  ...
  void tractionDeacy(const Real &time_at_failure, const Real &a_at_failure,
    ...
    for (unsigned int i = 0; i < 3; i++) {
      for (unsigned int j = 0; j < 3; j++) {
        _traction_derivatives[_qp](i, j) = 0;
        if (i == j)
          _traction_derivatives[_qp](i, j) = K[i];
      }
    }
  }
