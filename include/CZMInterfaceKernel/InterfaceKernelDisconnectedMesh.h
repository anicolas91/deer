...
#define TemplateVariableTestGradient                                           \
  typename OutputTools<T>::VariableTestGradient

// Forward Declarations
template <typename T> class InterfaceKernelTemplDisconnectedMesh;

typedef InterfaceKernelTemplDisconnectedMesh<Real>
    InterfaceKernelDisconnectedMesh;
typedef InterfaceKernelTemplDisconnectedMesh<RealVectorValue>
    VectorInterfaceKernelDisconnectedMesh;

template <> InputParameters validParams<InterfaceKernelDisconnectedMesh>();

template <>
InputParameters validParams<VectorInterfaceKernelDisconnectedMesh>();

/**
 * InterfaceKernelDisconnectedMesh and VectorInterfaceKernelDisconnectedMesh is
 * responsible for interfacing physics across subdomains
 */
template <typename T>
class InterfaceKernelTemplDisconnectedMesh
    : public InterfaceKernelBase,
      public NeighborMooseVariableInterface<T> {
public:
  static InputParameters validParams();

  InterfaceKernelTemplDisconnectedMesh(const InputParameters &parameters);
  ...
  /**
   * Using the passed DGResidual type, selects the correct test function space
   * and residual block, and then calls computeQpResidual
   */
  virtual void computeElemNeighResidual(Moose::DGResidualType type) override;

  /**
   * Using the passed DGJacobian type, selects the correct test function and
   * trial function spaces and jacobian block, and then calls computeQpJacobian
   */
  virtual void computeElemNeighJacobian(Moose::DGJacobianType type) override;

  /**
   * Using the passed DGJacobian type, selects the correct test function and
   * trial function spaces and jacobian block, and then calls
   * computeQpOffDiagJacobian with the passed jvar
   */
  virtual void computeOffDiagElemNeighJacobian(Moose::DGJacobianType type,
                                               unsigned int jvar) override;

  /// Selects the correct Jacobian type and routine to call for the master
  /// variable jacobian
  virtual void computeElementOffDiagJacobian(unsigned int jvar) override;

  /// Selects the correct Jacobian type and routine to call for the slave
  /// variable jacobian
...
protected:
  ...
  /// Holds the current solution gradient at the current quadrature point on the
  /// face.
  ...
  /// Holds residual entries as they are accumulated by this
  /// InterfaceKernelDisconnectedMesh This variable is temporarily reserved for
  /// RattleSnake
  DenseMatrix<Number> _local_kxx;

  /// Neighbor Jacobian/quadrature weight
  const MaterialProperty<Real> &_JxW_neighbor;
};
