...
// adot::adot(const std::string &fname, const Real &h, const Real &n,
//            const Real &D)
//     : nlFunRate(fname), _vdot(vdot("vdot", D, n, h)), _h(h){};
...
adot::io_maps_type adot::computeParamGradient(const io_maps_type &x,
  ...
  // if (!_eqTest.growthIsActive(x, params, x_old))
  //   return _empty_map;
  // Real dfdv = dfdvdotFun(x);
  // io_maps_type dv_dp = _vdot.computeParamGradient(x, params, x_old);
  // io_maps_type f_grad = chain(dfdv, dv_dp);
  // return f_grad;
  return _empty_map;
  ;
}
