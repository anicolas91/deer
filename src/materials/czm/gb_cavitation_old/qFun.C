...
// qFun::qFun(const std::string &fname)
//     : nlFunBase(fname, requiredVar(), requiredParam()), _f(f_ab("fab")) {}
//
// qFun::qFun(const std::string &fname, const Real &D)
//     : nlFunBase(fname, requiredVar(), requiredParam()), _f(f_Low("fLow", D))
//     {}

Real qFun::computeValue(const io_maps_type &x, const io_maps_type &params,
                        const io_maps_type &x_old) const {
  UNUSED(x_old);
  // _f.computeAndSetValue(x, params);
...
qFun::io_maps_type qFun::computeParamGradient(const io_maps_type &x,
  ...
  UNUSED(x_old);
  // Real dq_df = dqdf(x, params, x_old);
  // io_maps_type dq_dx = chain(dq_df, _f.computeParamGradient(x, params,
  // x_old)); return dq_dx;
  return _empty_map;
}
