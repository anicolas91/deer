...
InputParameters GBCavitation::validParams() {
  ...
  params.addRequiredParam<UserObjectName>(
      "interface_avg_mises_stress_uo",
      "name of theUO representing the average mises stress");
  params.addRequiredParam<UserObjectName>(
      "interface_avg_hydrostatic_stress_uo",
      "name of the UO representing the average mises stress");
  params.addRequiredParam<UserObjectName>(
  ...
  params.addParam<int>(
      "vdot_type", 3,
      "1 VL, 2 VH,  3(default)0  max(VL,VH), 4 average (VL+VH)/2");
  params.addParam<int>(
...
GBCavitation::GBCavitation(const InputParameters &parameters)
    : CZMMaterialBaseStateful(parameters), _a0(getParam<Real>("a_0")),
      ...
      _a_old(getMaterialPropertyOld<Real>("cavity_half_radius")),
      _b(declareProperty<Real>("cavity_half_spacing")),
      _b_old(getMaterialPropertyOld<Real>("cavity_half_spacing")),
      ...
      _avg_mises_stress_uo(
          getUserObject<InterfaceQpMaterialPropertyRankTwoScalarUO>(
              "interface_avg_mises_stress_uo")),
...
GBCavitation::~GBCavitation() { _q_ierr = SNESDestroy(&_q_snes); }

void GBCavitation::initQpStatefulProperties() {
  CZMMaterialBaseStateful::initQpStatefulProperties();
  ...
  if (_t_step > 0) {
    ...
    if (!_elem_failed_old[_qp]) {
      ...
      _avg_hyd_stress[_qp] = _avg_hyd_stress_uo.getSideAverageValue(
          _current_elem->id(), _current_side);
      _avg_eq_strain_rate[_qp] = _avg_eq_strain_rate_uo.getSideAverageValue(
          _current_elem->id(), _current_side);
...
.  f - function vector
 */
PetscErrorCode GBCavitation::FormFunction1(SNES q_snes, Vec x, Vec f,
                                           void *ctx) {
...
*/
PetscErrorCode GBCavitation::FormJacobian1(SNES q_snes, Vec x, Mat jac, Mat B,
                                           void *ctx) {
  ApplicationCtx *user = (ApplicationCtx *)ctx;
  nlFunBase::io_maps_type *xold_petsc = user->my_xold;
  nlFunBase::io_maps_type *params_petsc = user->my_params;
...
void GBCavitation::writeDataToFile() const {
...
void GBCavitation::setNewton(const bool &linesearch_on) {
  /* set solver type*/
...
void GBCavitation::updateLastSolution(
...
void GBCavitation::updatedStateVarFromRealSolution(
...
nlFunBase::io_maps_type GBCavitation::xoldFromOld() const {
...
void GBCavitation::decoupeldShearTraction(const Real &dt) {
  ...
  _traction_derivatives[_qp](1, 0) = 0;

  _traction_derivatives[_qp](1, 1) =
      eta / dt - std::exp(-dt / (eta * S)) * eta / dt;
  _traction_derivatives[_qp](1, 2) = 0;
  ...
  _traction_derivatives[_qp](2, 0) = 0;
  _traction_derivatives[_qp](2, 1) = 0;
  _traction_derivatives[_qp](2, 2) =
      eta / dt - std::exp(-dt / (eta * S)) * eta / dt;
}
