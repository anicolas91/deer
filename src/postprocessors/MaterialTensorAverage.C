...
registerMooseObject("DeerApp", MaterialTensorAverage);

InputParameters MaterialTensorAverage::validParams() {
  InputParameters params = ElementAveragePostprocessor::validParams();
  params.addClassDescription(
      "This postprocessor computes an element average of "
...
MaterialTensorAverage::MaterialTensorAverage(const InputParameters &parameters)
    : ElementAveragePostprocessor(parameters),
      _tensor(getMaterialProperty<RankTwoTensor>("rank_two_tensor")),
      _i(getParam<unsigned int>("index_i")),
      _j(getParam<unsigned int>("index_j")) {}

Real MaterialTensorAverage::computeQpIntegral() {
