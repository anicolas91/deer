...
// MOOSE includes
#include "INSDomainIntegralAction.h"
#include "Factory.h"
#include "FEProblem.h"
#include "Parser.h"
...
registerMooseAction("DeerApp", INSDomainIntegralAction, "add_user_object");

registerMooseAction("DeerApp", INSDomainIntegralAction, "add_aux_variable");

registerMooseAction("DeerApp", INSDomainIntegralAction, "add_aux_kernel");

registerMooseAction("DeerApp", INSDomainIntegralAction, "add_postprocessor");

registerMooseAction("DeerApp", INSDomainIntegralAction, "add_vector_postprocessor");

registerMooseAction("DeerApp", INSDomainIntegralAction, "add_material");

InputParameters
INSDomainIntegralAction::validParams()
{
  ...
  // params.addParam<std::vector<VariableName>>("velocity","the velocity");
...
INSDomainIntegralAction::INSDomainIntegralAction(const InputParameters & params)
  : Action(params),
...
INSDomainIntegralAction::~INSDomainIntegralAction() {}

void
INSDomainIntegralAction::act()
{
  ...
  else if (_current_task == "add_aux_kernel" && _output_q)
  {
    ...
    for (unsigned int ring_index = 0; ring_index < nrings; ++ring_index)
    {
      if (_q_function_type == GEOMETRY)
      {
        ...
        // Moose::out<<"j_integral_radius_inner "<<_radius_inner[ring_index]<<"\n";
        // Moose::out<<"j_integral_radius_outer: "<<_radius_outer[ring_index]<<"\n";
      }
      else if (_q_function_type == TOPOLOGY)
      {
      ...
      if (_treat_as_2d)
      {
        ...
        // Moose::out<<"ak_type_name: "<<ak_type_name<<"\n";
        // Moose::out<<"ak_name_stream.str(): "<<ak_name_stream.str()<<"\n";
      }
      else
      {
        for (unsigned int cfp_index = 0; cfp_index < num_crack_front_points; ++cfp_index)
  ...
  else if (_current_task == "add_vector_postprocessor")
  {
    ...
    {
      // Moose::out<<"Im running the postprocessor "<<"\n";
      ...
      // Moose::out<<"vpp_type_name : "<<vpp_type_name<<"\n";
      params.set<UserObjectName>("crack_front_definition") = uo_name;
      ...
      // Moose::out<<"jintegral_selection : "<<jintegral_selection<<"\n";
      params.set<ExecFlagEnum>("execute_on") = {EXEC_INITIAL, EXEC_TIMESTEP_END};
  ...
  else if (_current_task == "add_material")
  {
    ...
    if (have_j_integral)
    {
      ...
      const std::string mater_type_name2("INSEshelbyTensornew"); //this runs every time
      mater_name2 = "INSEshelbyTensornew";
      ...
      // _velocity = getParam<std::vector<VariableName>>("velocity");
      // params2.set<std::vector<VariableName>>("velocity") = _velocity;
...
unsigned int
INSDomainIntegralAction::calcNumCrackFrontPoints()
{
  ...
  // Moose::out<<"num_points : "<<num_points<<"\n";
  return num_points;
}
