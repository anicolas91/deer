...
#include "InterfaceKernelDisconnectedMesh.h"
...
template <typename T>
InputParameters InterfaceKernelTemplDisconnectedMesh<T>::validParams() {
  InputParameters params = InterfaceKernelBase::validParams();
  if (std::is_same<T, Real>::value)
    params.registerBase("InterfaceKernelDisconnectedMesh");
  else if (std::is_same<T, RealVectorValue>::value)
    params.registerBase("VectorInterfaceKernelDisconnectedMesh");
  else
    ::mooseError(
        "unsupported InterfaceKernelTemplDisconnectedMesh specialization");
  return params;
}

template <typename T>
InterfaceKernelTemplDisconnectedMesh<T>::InterfaceKernelTemplDisconnectedMesh(
    const InputParameters &parameters)
    : InterfaceKernelBase(parameters),
      NeighborMooseVariableInterface<T>(
          this, false, Moose::VarKindType::VAR_NONLINEAR,
          std::is_same<T, Real>::value ? Moose::VarFieldType::VAR_FIELD_STANDARD
                                       : Moose::VarFieldType::VAR_FIELD_VECTOR),
      _var(*this->mooseVariable()), _normals(_assembly.normals()),
      _u(_is_implicit ? _var.sln() : _var.slnOld()),
      _grad_u(_is_implicit ? _var.gradSln() : _var.gradSlnOld()),
      _phi(_assembly.phiFace(_var)), _grad_phi(_assembly.gradPhiFace(_var)),
      _test(_var.phiFace()), _grad_test(_var.gradPhiFace()),
      _neighbor_var(*getVarHelper<T>("neighbor_var", 0)),
      _neighbor_value(_is_implicit ? _neighbor_var.slnNeighbor()
                                   : _neighbor_var.slnOldNeighbor()),
      ...
      _JxW_neighbor(getMaterialPropertyByName<Real>("NeighborArea"))

{
  addMooseVariableDependency(this->mooseVariable());

  if (!parameters.isParamValid("boundary"))
    mooseError("In order to use an interface kernel, you must specify a "
  ...
  if (parameters.isParamSetByUser("save_in")) {
    if (_save_in_strings.size() != _save_in_var_side.size())
      ...
      for (unsigned i = 0; i < _save_in_strings.size(); ++i) {
        MooseVariable *var =
            &_subproblem.getStandardVariable(_tid, _save_in_strings[i]);
        ...
        if (_save_in_var_side[i] == "m") {
          if (var->feType() != _var.feType())
            mooseError("Error in " + name() +
                       ". There is a mismatch between the fe_type of the "
  ...
  _has_master_residuals_saved_in =
      _master_save_in_residual_variables.size() > 0;
  ...
  if (parameters.isParamSetByUser("diag_save_in")) {
    if (_diag_save_in_strings.size() != _diag_save_in_var_side.size())
      ...
      for (unsigned i = 0; i < _diag_save_in_strings.size(); ++i) {
        MooseVariable *var =
            &_subproblem.getStandardVariable(_tid, _diag_save_in_strings[i]);

        if (_sys.hasVariable(_diag_save_in_strings[i]))
          mooseError("Trying to use solution variable " +
                     _diag_save_in_strings[i] + " as a save_in variable in " +
                     name());
  ...
  _has_master_jacobians_saved_in =
      _master_save_in_jacobian_variables.size() > 0;
  _has_slave_jacobians_saved_in = _slave_save_in_jacobian_variables.size() > 0;
}

template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeElemNeighResidual(
  ...
  for (_qp = 0; _qp < _qrule->n_points(); _qp++)
    for (_i = 0; _i < test_space.size(); _i++) {
      // std::cout << _qp << " _JxW[_qp] " << _JxW[_qp] << " _JxW_neighbor[_qp]
      // "
      //           << _JxW_neighbor[_qp] << std::endl;
      if (is_elem)
...
template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeResidual() {
...
template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeElemNeighJacobian(
    ...
    for (_i = 0; _i < test_space.size(); _i++)
      for (_j = 0; _j < loc_phi.size(); _j++) {
        if (type == Moose::ElementElement || type == Moose::NeighborElement)
          _local_ke(_i, _j) +=
              _JxW[_qp] * _coord[_qp] * computeQpJacobian(type);
        else
          _local_ke(_i, _j) +=
              _JxW_neighbor[_qp] * _coord[_qp] * computeQpJacobian(type);
      }
...
template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeJacobian() {
  computeElemNeighJacobian(Moose::ElementElement);
  computeElemNeighJacobian(Moose::NeighborNeighbor);
}

template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeOffDiagElemNeighJacobian(
    Moose::DGJacobianType type, unsigned int jvar) {
  const TemplateVariableTestValue &test_space =
      (type == Moose::ElementElement || type == Moose::ElementNeighbor)
          ? _test
          : _test_neighbor;
  ...
  // Prevent calling of Jacobian computation if jvar doesn't lie in the current
  // block
    ...
    for (_qp = 0; _qp < _qrule->n_points(); _qp++)
      for (_i = 0; _i < test_space.size(); _i++)
        for (_j = 0; _j < loc_phi.size(); _j++) {
          if (type == Moose::ElementElement || type == Moose::NeighborElement)
            _local_ke(_i, _j) +=
                _JxW[_qp] * _coord[_qp] * computeQpOffDiagJacobian(type, jvar);
          else
            _local_ke(_i, _j) += _JxW_neighbor[_qp] * _coord[_qp] *
...
template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeElementOffDiagJacobian(
    unsigned int jvar) {
...
template <typename T>
void InterfaceKernelTemplDisconnectedMesh<T>::computeNeighborOffDiagJacobian(
    unsigned int jvar) {
  bool is_jvar_not_interface_var = true;
  if (jvar == _var.number()) {
...
// Explicitly instantiates the two versions of the
// InterfaceKernelTemplDisconnectedMesh class
template class InterfaceKernelTemplDisconnectedMesh<Real>;
template class InterfaceKernelTemplDisconnectedMesh<RealVectorValue>;
